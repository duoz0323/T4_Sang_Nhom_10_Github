...
import stu.edu.Backend_Nhom10.mapper.CompanyProfileMapper;
...
public class CompanyProfileService {
    ...
    @PreAuthorize("hasAnyRole('Company', 'COMPANY')")
    ...
    @PreAuthorize("hasAnyRole('Company', 'COMPANY')")
    public CompanyProfileResponse updateMyProfile(CompanyProfileRequest request) {
        var authentication = SecurityContextHolder.getContext().getAuthentication();
        String userId = authentication.getName();
        ...
        //không update keycloak vì chỉ quản lý email và password

        return profileMapper.toProfileResponse(profile);
    }

    public CompanyProfileResponse register(RegistrationCompanyRequest request) {
        try {
            String token = getAdminToken();
            var creationResponse = identityClient.createUser(
                    "Bearer " + token,
                    UserCreationParam.builder()
                            //đồng bộ username = email
            ...
            // Gán role COMPANY cho user
            try {
                // Lấy thông tin RoleCOMPANY từ Keycloak
                RoleRepresentation companyRole = identityClient.getRoleByName("Bearer " + token, "COMPANY");
                ...
                log.info("Đã gán thành công Role COMPANY cho User {}", userId);
            } catch (Exception e) {
    ...
    public void deleteProfile(String profileId) {
        CompanyProfile companyProfile = profileRepository.findById(profileId)
        ...
        profileRepository.delete(companyProfile);
    ...
    @PreAuthorize("hasAnyRole('Company', 'COMPANY')")
    public CompanyProfileResponse getProfileById(@Valid String profileId) {
